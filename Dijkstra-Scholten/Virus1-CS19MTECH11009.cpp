...
int Ir ,  Ib ,n;
float Wr,Wb, lamda_red, lamda_blue, lamda_send,p , q ;
...
int main()
{
		  ...
		  */
          ...
          newfile.open("inp-params.txt",ios::in); //open a file to perform read operation using file object
          int i=0, l=0;
          ...
          n=stoi(sep[0]);Wr=stof(sep[1]);Ir=stoi(sep[2]);Wb=stof(sep[3]);Ib=stoi(sep[4]);lamda_blue=stof(sep[5]);lamda_red=stof(sep[6]);
          lamda_send=stof(sep[7]);
          p=stof(sep[8]);q=stof(sep[9]);
            l=12;
          }
